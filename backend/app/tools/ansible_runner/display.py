...
class AdHocDisplay(Display, metaclass=UnSingleton):
    def __init__(self, execution_id, verbosity=0):
        ...
        # P0-LOW-CLEANUP: 原代码残留 print(1) 调试, 且 log_path 被注释但仍被 open 使用 -> NameError
        #   修复: 若有 execution_id 则启用日志文件 (devnull 之前), 否则丢弃到 devnull
        if execution_id:
            # log_path = get_ansible_task_log_path(execution_id)
            log_path = os.devnull  # REV18 临时: 待 get_ansible_task_log_path 实现后切换
        else:
            log_path = os.devnull
